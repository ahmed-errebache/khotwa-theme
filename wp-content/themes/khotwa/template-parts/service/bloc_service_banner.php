<?php
   ...
   $img_service = is_array($banner_service_image) && isset($banner_service_image['url']) ? esc_url($banner_service_image['url']) : esc_url($banner_service_image);
...
<section class="intro-section py-5 <?= esc_attr($direction_class); ?>">
  <div class="container">
    <div class="row align-items-center">
      <!-- Col image -->
      <div class="col-lg-6 mb-4 mb-lg-0 <?= $is_rtl ? 'order-lg-2' : 'order-lg-1' ?>">
        <?php if ($img_service): ?>
          <img src="<?php echo $img_service; ?>" alt="img service" class="img-fluid rounded mx-auto">
        <?php endif; ?>
      </div>
      ...
      <div class="col-lg-6">
        <?php if ($service_banner_title): ?>
          <h2 class="intro-title mb-3"><?= esc_html($service_banner_title); ?></h2>
        <?php endif; ?>

        <?php if ($service_banner_subtitle): ?>
          <h4 class="intro-subtitle mb-4"><?= esc_html($service_banner_subtitle); ?></h4>
        <?php endif; ?>

        <?php if ($service_points): ?>
          <ul class="list-unstyled mt-5">
            <?php foreach ($service_points as $service):
                $service_list = $service['service_list'];
                $service_icon = $service['service_icon'];
                $service_icon_url = is_array($service_icon) && isset($service_icon['url']) ? esc_url($service_icon['url']) : esc_url($service_icon);
            ?>          
              <li class="d-flex align-items-start service-item mb-3">
                <?php if (!empty($service_icon_url)): ?>
                  <img src="<?php echo $service_icon_url ?>" alt="service icon" class="icon_service">
                <?php endif; ?>
                <span><?php echo $service_list ?></span>
              </li>
            <?php endforeach; ?>
          </ul>
        <?php endif; ?>
